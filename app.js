/* ===== APP.JS completo (con 4 nuevas vistas) ===== */
(() => {
  // ---------- Utilidades y fechas (con fallback) ----------
  ...
  const D = {
    ...
    ymd: (s) => hasDayjs ? dayjs(s||undefined).format('YYYY-MM-DD') : (s?new Date(s):new Date()).toISOString().slice(0,10),
    dmY: (s) => hasDayjs ? dayjs(s).format('DD/MM/YYYY') : (()=>{const d=s?new Date(s):new Date();return String(d.getDate()).padStart(2,'0')+'/'+String(d.getMonth()+1).padStart(2,'0')+'/'+d.getFullYear()})(),
  ...
  const cfgLogo=$('#cfgLogo'); if(cfgLogo){ cfgLogo.addEventListener('change',(e)=>{const f=e.target.files?.[0]; if(!f) return; const r=new FileReader(); r.onload=()=>{logoData=r.result;LS.rawSet(KEY_LOGO,logoData);refreshBrand();}; r.readAsDataURL(f);});}
  ...
  function renderPayLinks(){
    ...
    payLinks.links.forEach(pl=>{
      ...
      row.querySelector('[data-def]').addEventListener('click',()=>{payLinks.defaultId=pl.id;LS.set(KEY_PL,payLinks);renderPayLinks();});
      row.querySelector('[data-del]').addEventListener('click',()=>{payLinks.links=payLinks.links.filter(x=>x.id!==pl.id); if(payLinks.defaultId===pl.id) payLinks.defaultId=null; LS.set(KEY_PL,payLinks); renderPayLinks();});
  ...
  // Modal para elegir enlace en factura
  ...
  bind('#modalPLClose',()=>modalPL.style.display='none');

  // ---------- Contact Picker + vCard ----------
  ...
  async function pickContactsAndSave({selectFor='clients'}){
    if(!(await supportsContactPicker())){alert('El Contact Picker no está disponible.');return;}
    try{
      ...
      clients.push(client); LS.set(KEY_CLI,clients); renderClients();
  ...
  bind('#importFromContacts',()=>pickContactsAndSave({selectFor:'clients'}));
  bind('#pickContactInline', ()=>pickContactsAndSave({selectFor:'invoice'}));
  // vCard
  ...
  bind('#importVCF',()=>$('#vcfInput').click());
  const vcf=$('#vcfInput'); if(vcf){ vcf.addEventListener('change',async(e)=>{const f=e.target.files?.[0]; if(!f) return;
      try{ const text=await f.text(); const parsed=parseVCardText(text); if(!parsed.length){alert('No se encontró ningún contacto válido.'); e.target.value=''; return;}
        parsed.forEach(nc=>{const c={id:'c_'+Date.now()+Math.floor(Math.random()*1000),...nc}; clients.push(c); const sel=$('#clientSelect'); if(sel){const o=document.createElement('option'); o.value=c.id; o.textContent=c.name||'Cliente'; sel.appendChild(o);} });
        LS.set(KEY_CLI,clients); renderClients(); const last=clients[clients.length-1]; if(last && $('#clientSelect')) $('#clientSelect').value=last.id; alert(`Importados ${parsed.length} contacto(s).`);
      }catch{alert('No se pudo leer el archivo .vcf');} finally{e.target.value='';}
  });}

  // ---------- Clientes ----------
  function renderClientSelect(){
    const sel=$('#clientSelect'); if(!sel) return; sel.innerHTML=''; const o0=document.createElement('option'); o0.value=''; o0.textContent='— Selecciona cliente —'; sel.appendChild(o0);
    clients.forEach(c=>{const o=document.createElement('option'); o.value=c.id; o.textContent=c.name; sel.appendChild(o);});
  }
  function renderClients(){
    renderClientSelect(); const w=$('#clientsList'); if(!w) return; w.innerHTML=''; if(!clients.length){w.innerHTML='<p class="muted">Sin clientes guardados.</p>'; return;}
    [...clients].reverse().forEach(c=>{const row=document.createElement('div'); row.className='item';
      ...
      row.querySelector('[data-del]').addEventListener('click',()=>{clients=clients.filter(x=>x.id!==c.id);LS.set(KEY_CLI,clients);renderClients();});
      w.appendChild(row);
    });
  }
  bind('#saveClient',()=>{const c={id:'c_'+Date.now(),name:($('#cName').value||'').trim(),email:($('#cEmail').value||'').trim(),phone:($('#cPhone').value||'').trim(),address:($('#cAddress').value||'').trim()};
    if(!c.name){alert('Nombre es requerido.');return;} clients.push(c); LS.set(KEY_CLI,clients); renderClients(); $('#clientForm').reset(); alert('Cliente guardado.');});
  // Modal cliente rápido
  bind('#addClientInline',()=>{$('#quickClientForm').reset?.(); $('#modalClient').style.display='flex'; setTimeout(()=>$('#qcName')?.focus(),50);});
  bind('#qcCancel',()=>$('#modalClient').style.display='none');
  bind('#qcSave',()=>{const c={id:'c_'+Date.now(),name:($('#qcName').value||'').trim(),email:($('#qcEmail').value||'').trim(),phone:($('#qcPhone').value||'').trim(),address:($('#qcAddress').value||'').trim()};
    if(!c.name){alert('Nombre es requerido.');return;} clients.push(c); LS.set(KEY_CLI,clients); renderClients(); const sel=$('#clientSelect'); const o=document.createElement('option'); o.value=c.id; o.textContent=c.name; sel.appendChild(o); sel.value=c.id; $('#modalClient').style.display='none';});

  // ---------- Factura / líneas / totales ----------
  ...
  bind('#addLine',()=>addLine());
  $('#invDate').value=D.todayISO(); $('#invDue').value=D.plusDaysISO(7); addLine();

  // Añadir desde catálogo
  bind('#fromCatalog',()=>{ if(!products.length){alert('No hay productos en el catálogo.'); return;} show('view-catalog'); });
  ...
  bind('#btnSave',()=>{const inv=getInvoiceFromForm(); if(!inv.lines.length){alert('Agrega al menos una línea.');return;} invoices.push(inv); LS.set(KEY_INV,invoices); $('#invNumber').value=inv.number+1; renderHistory(); renderHistorySummary(); alert('Factura guardada.');});
  bind('#btnClear',()=>{$('#invoiceForm').reset(); linesBody.innerHTML=''; addLine(); $('#invDate').value=D.todayISO(); $('#invDue').value=D.plusDaysISO(7); autoFillDefaultPayLink(); recalc();});
  ...
  document.addEventListener('change',(e)=>{if(e.target?.id==='histMonth') renderHistorySummary();},{passive:true});
  function renderHistory(){
    const w=$('#historyList'); w.innerHTML=''; if(!invoices.length){w.innerHTML='<p class="muted">Sin facturas guardadas.</p>'; renderHistorySummary(); return;}
    ...
    sorted.forEach(inv=>{const code=`${inv.prefix}${String(inv.number).padStart(4,'0')}`; const row=document.createElement('div'); row.className='item';
      ...
      row.querySelector('[data-del]').addEventListener('click',()=>{invoices=invoices.filter(x=>x.id!==inv.id); LS.set(KEY_INV,invoices); renderHistory(); renderHistorySummary();});
  ...
  function renderProducts(){
    ...
    [...products].reverse().forEach(p=>{const row=document.createElement('div'); row.className='item';
      ...
      row.querySelector('[data-del]').addEventListener('click',()=>{products=products.filter(x=>x.id!==p.id); LS.set(KEY_PRD,products); renderProducts();});
      w.appendChild(row);
    });
  }
  bind('#addProduct',()=>{const name=$('#pName').value.trim(); const price=parseFloat($('#pPrice').value)||0; if(!name){alert('Nombre requerido.');return;} const p={id:'p_'+Date.now(),name,price}; products.push(p); LS.set(KEY_PRD,products); $('#pName').value=''; $('#pPrice').value=''; renderProducts(); alert('Guardado en catálogo.');});

  // ---------- Pagos rápidos ----------
  function renderQuickPay(){
    payLinks=LS.get(KEY_PL,{links:[],defaultId:null});
    const sel=$('#qpLink'); const info=$('#qpInfo'); sel.innerHTML='';
    if(!payLinks.links.length){ info.innerHTML='<p class="muted">No hay enlaces guardados. Agrega en Configuración.</p>'; return; }
    ...
    info.innerHTML='<p class="muted">Selecciona un enlace, escribe el monto y abre la página de pago.</p>';
  }
  bind('#qpOpen',()=>{const id=$('#qpLink').value; const amt=parseFloat($('#qpAmount').value)||0; const pl=payLinks.links.find(x=>x.id===id); if(!pl){alert('Elige un enlace');return;}
    // abrimos el enlace tal cual; si en el futuro soporta monto por query, se añadirá aquí.
    window.open(pl.url,'_blank');
  });
  bind('#qpCopy',async()=>{const id=$('#qpLink').value; const pl=payLinks.links.find(x=>x.id===id); if(!pl) return; try{await navigator.clipboard.writeText(pl.url); alert('Enlace copiado.');}catch{alert('No se pudo copiar.');}});

  // ---------- Reportes (sin librerías) ----------
  function renderReports(){
    // por mes
    const wm=$('#repMonths'); wm.innerHTML='';
    const byMonth={}; invoices.forEach(i=>{const k=D.monthKey(i.date); byMonth[k]=(byMonth[k]||0)+(i.totals?.total||0);});
    const keys=Object.keys(byMonth).sort().reverse(); if(!keys.length){wm.innerHTML='<p class="muted">Sin datos.</p>';} else {
      keys.forEach(k=>{const row=document.createElement('div'); row.className='item'; row.innerHTML=`<div class="meta"><b>${D.monthLabel(k)}</b><small>Total ${fmt(byMonth[k])}</small></div>`; wm.appendChild(row);});
    }
    // por cliente
    const wc=$('#repClients'); wc.innerHTML='';
    const byClient={}; invoices.forEach(i=>{const name=i.clientName||'Cliente'; byClient[name]=(byClient[name]||0)+(i.totals?.total||0);});
    const arr=Object.entries(byClient).sort((a,b)=>b[1]-a[1]).slice(0,10);
    if(!arr.length){wc.innerHTML='<p class="muted">Sin datos.</p>';} else {
      arr.forEach(([n,t])=>{const row=document.createElement('div'); row.className='item'; row.innerHTML=`<div class="meta"><b>${n}</b><small>Total ${fmt(t)}</small></div>`; wc.appendChild(row);});
    }
  }

  // ---------- Notas rápidas ----------
  function renderTemplates(){
    const w=$('#tnList'); w.innerHTML=''; if(!templates.length){w.innerHTML='<p class="muted">Sin notas guardadas.</p>'; return;}
    [...templates].reverse().forEach(t=>{const row=document.createElement('div'); row.className='item';
      ...
      row.querySelector('[data-del]').addEventListener('click',()=>{templates=templates.filter(x=>x.id!==t.id); LS.set(KEY_TN,templates); renderTemplates();});
      w.appendChild(row);
    });
  }
  bind('#tnSave',()=>{const title=$('#tnTitle').value.trim(), body=$('#tnBody').value.trim(); if(!title||!body){alert('Título y contenido son requeridos.'); return;} const t={id:'tn_'+Date.now(), title, body}; templates.push(t); LS.set(KEY_TN,templates); $('#tnTitle').value=''; $('#tnBody').value=''; renderTemplates(); alert('Nota guardada.'); });
  ...
  (function init(){
    ...
    // selector de pago por defecto en factura
    autoFillDefaultPayLink();
  })();

  console.log('Factura Móvil — inicializado con 4 nuevos módulos.');
})();
