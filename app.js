...
(() => {
  ...
  $('#clientClearSearch')?.addEventListener('click', ()=>{ const inp=$('#clientSearch'); if(inp) inp.value=''; clientQuery=''; renderClients(); });
  ...
  bind('#qpOpen',()=>{const id=$('#qpLink').value; const pl=payLinks.links.find(x=>x.id===id); if(!pl){alert('Elige un enlace');return;} window.open(pl.url,'_blank');});
  ...
  function renderReports(){
    const wm=$('#repMonths'); wm.innerHTML=''; const byMonth={}; invoices.forEach(i=>{const k=D.monthKey(i.date); byMonth[k]=(byMonth[k]||0)+(i.totals?.total||0);});
    ...
    const wc=$('#repClients'); wc.innerHTML=''; const byClient={}; invoices.forEach(i=>{const name=i.clientName||'Cliente'; byClient[name]=(byClient[name]||0)+(i.totals?.total||0);});
  ...
  function renderWAList(){
    ...
    invoices.slice().sort((a,b)=>new Date(b.date)-new Date(a.date)).forEach(inv=>{
      ...
      o.value = inv.id;
      o.textContent = `${code} · ${inv.clientName||'Cliente'} · ${fmt(inv.totals?.total||0)}`;
      sel.appendChild(o);
    });
  ...
  bind('#waBuild',()=>buildWAText()); bind('#waOpen',()=>openWhatsApp()); bind('#waCopy',()=>copyText());
  ...
  bind('#bkExport',()=>downloadJSON(snapshotData()));
  bind('#bkImportBtn',()=>$('#bkFile').click());
  ...
  (function init(){
    ...
    $('#invDate').value=D.todayISO(); $('#invDue').value=D.plusDaysISO(7); autoFillDefaultPayLink();
  })();

  console.log('Factura Móvil — lista.');
})();
